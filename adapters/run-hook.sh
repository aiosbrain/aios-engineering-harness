...
STOP_POLICY=0
case "$POLICY" in
  guard-secrets.sh|guard-protected-paths.sh|guard-destructive.sh|guard-worktree.sh|guard-linear-routing.sh|outbound-comms-guard.sh|post-edit-format.sh) ;;
  stop-verify-gate.sh) STOP_POLICY=1 ;;
  inject-context.sh|route-skills.sh|route-aios-linear.sh) CONTEXT_POLICY=1 ;;
