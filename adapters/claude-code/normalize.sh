...
case "$EVENT" in
  ...
  pre_tool)
    NORMALIZED=$(printf '%s' "$INPUT" | jq -c --arg cwd "${PWD:-.}" '
      (.tool_input // {}) as $t |
      (.tool_name // .tool // "unknown") as $name |
      {
        protocol_version:"1.2", event:"pre_tool", runtime:{name:"claude"},
        cwd:(.cwd // $cwd), session_id:(.session_id // ""),
        tool_name:$name, tool_id:(.tool_use_id // .tool_call_id // ""),
        ...
        tool_input:({
          command:($t.command // $t.cmd), url:($t.url // $t.uri),
          method:$t.method, server:(.server_name // $t.server),
